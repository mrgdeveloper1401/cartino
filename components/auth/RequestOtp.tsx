...
import { requestOtp, RequestOtpValue } from "@/lib/validations/auth";
...
export function RequestOtpForm() {
  ...
  } = useForm<RequestOtpValue>({
    resolver: zodResolver(requestOtp),
    defaultValues: { phone_number: "" },
  });

  async function onSubmit(data: RequestOtpValue) {
    setServerError(null);
    try {
      const res = await fetch("/api/v2/auth/request_otp", {
        method: "POST",
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl border border-white/20 bg-white/10 p-8 shadow-2xl backdrop-blur-xl">
        <div className="mb-8 text-center">
          ...
          <p className="mt-2 text-sm text-white/60">
            شماره موبایل خود را وارد کنید تا کد تاید ارسال شود
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5" dir="rtl">
          ...
          <div className="flex justify-center text-white font-light">
            <Link href={"/login"}>ورود از طریق شماره موبایل و رمز عبور</Link>
          </div>
        </form>
      </div>
    </div>
